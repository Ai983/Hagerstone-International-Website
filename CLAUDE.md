...
| **Design page** (`/our-designs`) | Follow `docs/OUR-DESIGNS-INTAKE.md`. One `.mdx` in `src/content/design/`, images via `scripts/our-designs-images.mjs`. **Never name or show the client**; `designStage` stays `concept` unless a reviewer is named. |
